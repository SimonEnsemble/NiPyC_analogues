...
begin
	filenames = ["Ni-PYC_TGA.csv", "Ni-PYC-NH2_TGA.csv"]
	files = [joinpath.(pwd(), f_) for f_ in filenames]

	data = Dict{Symbol, Any}()
	for (i, mof_key) in enumerate([:nipyc, :nipycnh])
		data[mof_key] = CSV.read(files[i], DataFrame)
	end
	data
end
...
begin
	mof_to_label = Dict(:nipyc => "Ni(PyC)₂",
	...
	tight_layout()
	savefig("TGA_results.png", dpi=600, format="png")
	savefig("TGA_results.pdf", format="pdf")
	gcf()
end

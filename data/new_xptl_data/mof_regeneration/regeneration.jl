...
begin
	...
	function desorption_onset(sorption::Array{Float64, 1})
		des_index = sortperm(sorption, rev=true)[1]
		return des_index
	end
end
...
begin
	...
	for cycle in 1:5
		...
		data[key] = Dict(:sorption => xptl_data[!, "Quantity Adsorbed (mmol/g)"], 
			:abs_pressure => xptl_data[!, "Absolute Pressure (kPa)"])

		# add origin to data for plotting purposes
		pushfirst!(data[key][:sorption], 0.0)
		pushfirst!(data[key][:abs_pressure], 0.0)

		# unit conversion: 1 bar = 100 kPa
		data[key][:bar] = data[key][:abs_pressure] / 100
	end
	data
end

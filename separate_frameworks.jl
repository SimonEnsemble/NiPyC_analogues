...
write_xyz(f_replicated, "rep_NiPyC2_atoms.xyz")
...
for (comp_num, component) in enumerate(conn_comps)
    ...
    for i in 1:length(component)
        atoms_xf[:, i] .= f_replicated.atoms.xf[:, component[i]]
        atoms_species[i] = f_replicated.atoms.species[component[i]]
        if f_replicated.charges.n_charges > 0
            charges_q[i] = f_replicated.charges.q[component[i]]
            charges_xf[:, i] .= f_replicated.charges.xf[:, component[i]]
        end
    end
    if f_replicated.charges.n_charges > 0
    ...
    comp_f = Framework(comp_name, f_replicated.box, Atoms(atoms_species, atoms_xf),
